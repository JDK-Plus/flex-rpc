...
public class FlexClientFactory {
     ...
     */
    public FlexClientFactory(ClientRegistry clientRegistry) {
     ...
     * 获取指定类型的实例，并应用提供的拦截器。
     */
    public synchronized  <T> T acquireInstance(Class<T> injectionType, ClientInterceptor... interceptors) {
        ...
        T instance = dynamicProxy.acquireProxy(injectionType, (proxy, method, args) -> {
            ...
            List<ClientInterceptor> clientInterceptors = new ArrayList<>(List.of(interceptors));
